...
#   is_container_running()    — check if ralph-task-db-dev container is running
...
#   POSTGRES_PORT (optional, default 5466)
...
is_container_running() {
    docker inspect --format '{{.State.Running}}' ralph-task-db-dev 2>/dev/null | grep -q 'true'
}
...
wait_for_healthy() {
    ...
    while [ "$elapsed" -lt "$timeout" ]; do
        local health_status
        health_status=$(docker inspect --format '{{.State.Health.Status}}' ralph-task-db-dev 2>/dev/null) || true
        if [ "$health_status" = "healthy" ] && pg_isready -h localhost -p "${POSTGRES_PORT:-5466}" -q 2>/dev/null; then
            return 0
        fi
        sleep 1
        elapsed=$((elapsed + 1))
    done
    echo "Error: ralph-task-db-dev failed to become healthy within ${timeout}s"
    exit 1
}
