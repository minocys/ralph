...
- If the `ralph-task-dev` container is not running, `ralph.sh` starts it via `docker compose up -d` using `$SCRIPT_DIR/docker-compose.yml` with `--project-directory "$SCRIPT_DIR"` so the command works regardless of the caller's working directory.
...
- The PostgreSQL container listens on host port **5464** (not the default 5432) to avoid conflicts with any local PostgreSQL instance.
- The Docker service is named `ralph-task-dev` with `container_name: ralph-task-dev`.
