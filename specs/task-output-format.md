...
- `task list --all` must show all tasks including deleted (full DAG), using the human-readable table format by default (same columns as `task list`: ID, P, S, CAT, TITLE, AGENT)
- `task list --all --markdown` must output markdown-KV format for machine consumption
- Sort order is unchanged: priority ASC, then created_at ASC
- `plan-export` is deprecated in favor of `list --all` (see `specs/deprecate-plan-export.md`)
...
- `lib/loop.sh` must call `task list --all --markdown` in plan mode
...
- The table format for `task list` (default, no flags) is unchanged; `task list --all` uses the same table format
