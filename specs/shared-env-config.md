...
- A `.env.example` file is tracked in git with default values: `POSTGRES_USER=ralph`, `POSTGRES_PASSWORD=ralph`, `POSTGRES_DB=ralph`, `POSTGRES_PORT=5464`, `RALPH_DB_URL=postgres://ralph:ralph@localhost:5464/ralph`.
...
- `docker-compose.yml` references the `.env` file via `env_file: .env` and uses `${POSTGRES_PORT:-5464}` interpolation for the port mapping so compose works even without `.env`.
