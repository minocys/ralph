...
- After selecting, the builder must claim the task via `ralph task claim <id>` (targeted claiming — see task-scheduling spec)
- If `ralph task claim <id>` exits with code 2 (task no longer eligible — claimed by another agent, blocked, etc.), the builder should select the next best task from the snapshot and retry; if no claimable tasks remain, stop gracefully
...
- The builder must complete a task via `ralph task done <id> --result '{"commit":"<sha>"}'` where `<sha>` is the commit SHA from the git commit just made
- The builder must release a task on failure via `ralph task fail <id> --reason "<text>"` — this sets status back to `open`, clears the assignee, and increments `retry_count`
- The builder must use `ralph task create` to log discovered bugs or new work items as tasks instead of editing a JSON file
...
- The `result` JSON passed to `ralph task done` must include a `commit` key — this is how downstream tasks receive context about upstream changes
...
- Lease renewal (`ralph task renew`) — tasks should be small enough that 600 seconds suffices
- Retry count logic — ralph.sh does not react to retry_count
- Plan-phase operations (`ralph task plan-sync`, `ralph task list --all`)
