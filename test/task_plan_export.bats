...
@test "plan-export returns empty output with no tasks" {
    run bash -c "'$SCRIPT_DIR/lib/task' plan-export 2>/dev/null"
    assert_success
    assert_output ""
}
...
@test "plan-export defaults to table format" {
    "$SCRIPT_DIR/lib/task" create "pe-01" "First task" -p 1 -c "feat" > /dev/null
...
@test "plan-export table includes deleted tasks (full DAG)" {
    "$SCRIPT_DIR/lib/task" create "pe-alive" "Alive task" > /dev/null
    "$SCRIPT_DIR/lib/task" create "pe-dead" "Dead task" > /dev/null
    "$SCRIPT_DIR/lib/task" delete "pe-dead" > /dev/null
...
@test "plan-export table orders by priority ascending" {
    "$SCRIPT_DIR/lib/task" create "pe-low" "Low priority" -p 3 > /dev/null
    "$SCRIPT_DIR/lib/task" create "pe-high" "High priority" -p 0 > /dev/null
    "$SCRIPT_DIR/lib/task" create "pe-mid" "Mid priority" -p 1 > /dev/null
...
@test "plan-export table shows header columns" {
    "$SCRIPT_DIR/lib/task" create "pe-hdr" "Header test" -p 1 -c "feat" > /dev/null
...
@test "plan-export table shows assignee when set" {
    "$SCRIPT_DIR/lib/task" create "pe-agent" "Assigned task" > /dev/null
...
@test "plan-export --markdown outputs markdown-KV sections" {
    ...
    run "$SCRIPT_DIR/lib/task" plan-export --markdown
    assert_success
...
@test "plan-export --markdown omits null and empty fields" {
    ...
    "$SCRIPT_DIR/lib/task" create "pe-minimal" "Minimal task" > /dev/null
...
@test "plan-export --markdown includes steps and deps" {
    "$SCRIPT_DIR/lib/task" create "pe-blocker" "Blocker" > /dev/null
    "$SCRIPT_DIR/lib/task" create "pe-03" "Task with steps and deps" --deps "pe-blocker" > /dev/null
...
@test "plan-export --markdown separates tasks with blank lines" {
    "$SCRIPT_DIR/lib/task" create "pe-a" "Task A" -p 0 > /dev/null
    "$SCRIPT_DIR/lib/task" create "pe-b" "Task B" -p 1 > /dev/null
...
@test "plan-export --markdown returns empty output with no tasks" {
    run bash -c "'$SCRIPT_DIR/lib/task' plan-export --markdown 2>/dev/null"
    assert_success
    assert_output ""
}
...
@test "plan-export shows all statuses unlike list" {
    "$SCRIPT_DIR/lib/task" create "pe-open" "Open" > /dev/null
    "$SCRIPT_DIR/lib/task" create "pe-del" "Deleted" > /dev/null
    "$SCRIPT_DIR/lib/task" delete "pe-del" > /dev/null
...
@test "plan-export rejects unknown flags" {
    run "$SCRIPT_DIR/lib/task" plan-export --invalid
    assert_failure
    assert_output --partial "unknown flag"
}

@test "plan-export rejects --json flag" {
    run "$SCRIPT_DIR/lib/task" plan-export --json
    assert_failure
    assert_output --partial "unknown flag"
}
...
@test "plan-export deprecation warning appears on stderr separately" {
    ...
    "$SCRIPT_DIR/lib/task" plan-export 2>"$stderr_file" || true
...
@test "plan-export stdout is not polluted by deprecation warning" {
    "$SCRIPT_DIR/lib/task" create "pe-clean" "Clean stdout test" -p 1 -c "feat" > /dev/null

    # Capture stdout only (discard stderr)
    local stdout_output
    stdout_output=$("$SCRIPT_DIR/lib/task" plan-export 2>/dev/null)
...
@test "plan-export --markdown stdout is not polluted by deprecation warning" {
    "$SCRIPT_DIR/lib/task" create "pe-md-clean" "Markdown clean test" -p 1 > /dev/null

    # Capture stdout and stderr separately
    local stdout_output stderr_file="$TEST_WORK_DIR/stderr_md.txt"
    stdout_output=$("$SCRIPT_DIR/lib/task" plan-export --markdown 2>"$stderr_file")
...
@test "list --all table output matches plan-export table stdout" {
    "$SCRIPT_DIR/lib/task" create "parity-open" "Open task" -p 1 -c "feat" > /dev/null
    "$SCRIPT_DIR/lib/task" create "parity-done" "Done task" -p 2 -c "test" > /dev/null
    ...
    "$SCRIPT_DIR/lib/task" create "parity-del" "Deleted task" -p 3 > /dev/null
    "$SCRIPT_DIR/lib/task" delete "parity-del" > /dev/null

    # Capture stdout only from both commands
    local pe_stdout la_stdout
    pe_stdout=$("$SCRIPT_DIR/lib/task" plan-export 2>/dev/null)
    la_stdout=$("$SCRIPT_DIR/lib/task" list --all 2>/dev/null)
...
@test "list --all --markdown output matches plan-export --markdown stdout" {
    "$SCRIPT_DIR/lib/task" create "mdpar-open" "Open task" -p 1 -c "feat" > /dev/null
    "$SCRIPT_DIR/lib/task" create "mdpar-done" "Done task" -p 2 -c "test" > /dev/null
    ...
    "$SCRIPT_DIR/lib/task" create "mdpar-del" "Deleted task" -p 3 > /dev/null
    "$SCRIPT_DIR/lib/task" delete "mdpar-del" > /dev/null
    "$SCRIPT_DIR/lib/task" create "mdpar-blocker" "Blocker" > /dev/null
    "$SCRIPT_DIR/lib/task" create "mdpar-blocked" "Blocked task" -p 0 --deps "mdpar-blocker" > /dev/null
    ...
    # Capture stdout only from both commands
    local pe_stdout la_stdout
    pe_stdout=$("$SCRIPT_DIR/lib/task" plan-export --markdown 2>/dev/null)
    la_stdout=$("$SCRIPT_DIR/lib/task" list --all --markdown 2>/dev/null)
