...
@test "task done on already done task exits 1" {
    ...
    sqlite3 "$RALPH_DB_PATH" "UPDATE tasks SET status='active', assignee='agent-1' WHERE slug='td-02' AND scope_repo='test/repo' AND scope_branch='main';"
...
@test "task done on active task succeeds" {
    ...
    sqlite3 "$RALPH_DB_PATH" "UPDATE tasks SET status='active', assignee='agent-1' WHERE slug='td-03' AND scope_repo='test/repo' AND scope_branch='main';"
...
@test "task done sets status to done" {
    ...
    sqlite3 "$RALPH_DB_PATH" "UPDATE tasks SET status='active', assignee='agent-1' WHERE slug='td-04' AND scope_repo='test/repo' AND scope_branch='main';"

    "$SCRIPT_DIR/lib/task" done "td-04"

    local task_status
    task_status=$(sqlite3 "$RALPH_DB_PATH" "SELECT status FROM tasks WHERE slug='td-04' AND scope_repo='test/repo' AND scope_branch='main';")
    [ "$task_status" = "done" ]
}

@test "task done sets updated_at" {
    "$SCRIPT_DIR/lib/task" create "td-05" "Active task"
    sqlite3 "$RALPH_DB_PATH" "UPDATE tasks SET status='active', assignee='agent-1', updated_at=NULL WHERE slug='td-05' AND scope_repo='test/repo' AND scope_branch='main';"

    "$SCRIPT_DIR/lib/task" done "td-05"

    local updated
    updated=$(sqlite3 "$RALPH_DB_PATH" "SELECT updated_at FROM tasks WHERE slug='td-05' AND scope_repo='test/repo' AND scope_branch='main';")
    [ -n "$updated" ]
}
...
@test "task done with --result stores JSON" {
    ...
    sqlite3 "$RALPH_DB_PATH" "UPDATE tasks SET status='active', assignee='agent-1' WHERE slug='td-06' AND scope_repo='test/repo' AND scope_branch='main';"
    ...
    local result
    result=$(sqlite3 "$RALPH_DB_PATH" "SELECT json_extract(result, '\$.output') FROM tasks WHERE slug='td-06' AND scope_repo='test/repo' AND scope_branch='main';")
    [ "$result" = "success" ]
}

@test "task done with --result missing commit key exits 1" {
    "$SCRIPT_DIR/lib/task" create "td-06b" "Task missing commit"
    sqlite3 "$RALPH_DB_PATH" "UPDATE tasks SET status='active', assignee='agent-1' WHERE slug='td-06b' AND scope_repo='test/repo' AND scope_branch='main';"
...
@test "task done with --result commit null exits 1" {
    ...
    sqlite3 "$RALPH_DB_PATH" "UPDATE tasks SET status='active', assignee='agent-1' WHERE slug='td-06c' AND scope_repo='test/repo' AND scope_branch='main';"
...
@test "task done without --result stores no result" {
    ...
    sqlite3 "$RALPH_DB_PATH" "UPDATE tasks SET status='active', assignee='agent-1' WHERE slug='td-07' AND scope_repo='test/repo' AND scope_branch='main';"

    "$SCRIPT_DIR/lib/task" done "td-07"

    local result
    result=$(sqlite3 "$RALPH_DB_PATH" "SELECT result FROM tasks WHERE slug='td-07' AND scope_repo='test/repo' AND scope_branch='main';")
    [ -z "$result" ]
}
...
@test "task done implicitly unblocks downstream tasks for claim" {
    ...
    sqlite3 "$RALPH_DB_PATH" "UPDATE tasks SET status='active', assignee='agent-test' WHERE slug='td-blocker' AND scope_repo='test/repo' AND scope_branch='main';"
...
@test "task done result is visible in show output" {
    ...
    sqlite3 "$RALPH_DB_PATH" "UPDATE tasks SET status='active', assignee='agent-1' WHERE slug='td-08' AND scope_repo='test/repo' AND scope_branch='main';"
...
@test "task done works with special characters in task ID" {
    ...
    sqlite3 "$RALPH_DB_PATH" "UPDATE tasks SET status='active', assignee='agent-1' WHERE slug='td/special-09' AND scope_repo='test/repo' AND scope_branch='main';"
...
@test "task done works with single quotes in task ID" {
    ...
    sqlite3 "$RALPH_DB_PATH" "UPDATE tasks SET status='active', assignee='agent-1' WHERE slug='td''quoted' AND scope_repo='test/repo' AND scope_branch='main';"
...
@test "task done with result containing special JSON characters" {
    ...
    sqlite3 "$RALPH_DB_PATH" "UPDATE tasks SET status='active', assignee='agent-1' WHERE slug='td-10' AND scope_repo='test/repo' AND scope_branch='main';"
