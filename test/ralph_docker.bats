...
#!/bin/bash
if [ "$1" = "inspect" ]; then
    echo "Error: No such object: ralph-task-db-dev" >&2
    exit 1
fi
exit 0
STUB
...
@test "is_container_running passes ralph-task-db-dev as container name to docker inspect" {
    cat > "$STUB_DIR/docker" <<'STUB'
#!/bin/bash
if [ "$1" = "inspect" ]; then
    # Log all args so we can verify the container name
    echo "DOCKER_ARGS: $*" >&2
    # Check the last argument is ralph-task-db-dev
    if [ "${@: -1}" = "ralph-task-db-dev" ]; then
        echo "true"
        exit 0
    fi
    exit 1
...
@test "wait_for_healthy checks docker health status format for ralph-task-db-dev" {
    cat > "$STUB_DIR/docker" <<'STUB'
#!/bin/bash
if [ "$1" = "inspect" ] && [ "$2" = "--format" ]; then
    # Verify format string and container name
    if [ "$3" = "{{.State.Health.Status}}" ] && [ "$4" = "ralph-task-db-dev" ]; then
        echo "healthy"
        exit 0
    fi
    exit 1
