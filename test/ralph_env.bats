...
@test "sourcing .env sets RALPH_DB_URL when not already set" {
    printf 'RALPH_DB_URL=postgres://ralph:ralph@localhost:5466/ralph\n' > "$TEST_WORK_DIR/.env"
    unset RALPH_DB_URL
    SCRIPT_DIR="$TEST_WORK_DIR"
    _source_env
    [ "$RALPH_DB_URL" = "postgres://ralph:ralph@localhost:5466/ralph" ]
...
@test "sourcing .env preserves existing RALPH_DB_URL" {
    printf 'RALPH_DB_URL=postgres://ralph:ralph@localhost:5466/ralph\n' > "$TEST_WORK_DIR/.env"
    RALPH_DB_URL="postgres://custom:custom@remotehost:5466/mydb"
    SCRIPT_DIR="$TEST_WORK_DIR"
    _source_env
    [ "$RALPH_DB_URL" = "postgres://custom:custom@remotehost:5466/mydb" ]
}
