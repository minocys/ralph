...
@test "plan-sync with empty stdin prints zero summary" {
    run bash -c 'echo "" | "$SCRIPT_DIR/lib/task" plan-sync'
    assert_success
...
{"id":"ps-02","t":"Second task","p":2,"cat":"feat","spec":"my-spec"}'
    ...
    run "$SCRIPT_DIR/lib/task" show ps-01
    assert_success
    assert_output --partial "First task"

    run "$SCRIPT_DIR/lib/task" show ps-02
    assert_success
...
@test "plan-sync inserts tasks with steps and dependencies" {
    ...
    "$SCRIPT_DIR/lib/task" create ps-dep-01 "Blocker task" -r my-spec >/dev/null
...
{"id":"ps-dep-02","t":"Dependent task","p":1,"spec":"my-spec","steps":["Step one","Step two"],"deps":["ps-dep-01"]}'
    ...
    # Verify deps
    run "$SCRIPT_DIR/lib/task" deps ps-dep-02
    assert_success
...
@test "plan-sync updates existing non-done tasks" {
    # Create a task first
    "$SCRIPT_DIR/lib/task" create ps-upd-01 "Original title" -p 2 -c feat -r my-spec >/dev/null
    ...
    run "$SCRIPT_DIR/lib/task" show ps-upd-01
    assert_success
...
{"id":"ps-rep-03","t":"Main task","spec":"my-spec","steps":["New step 1","New step 2"],"deps":["ps-rep-02"]}'
    ...
    run "$SCRIPT_DIR/lib/task" deps ps-rep-03
    assert_success
...
@test "plan-sync skips done tasks" {
    ...
    "$SCRIPT_DIR/lib/task" create ps-done-01 "Done task" -p 1 -r my-spec >/dev/null
    export RALPH_AGENT_ID="test-agent"
    "$SCRIPT_DIR/lib/task" claim --agent test-agent >/dev/null
    "$SCRIPT_DIR/lib/task" done ps-done-01 >/dev/null
    ...
    run "$SCRIPT_DIR/lib/task" show ps-done-01
    assert_success
...
@test "plan-sync soft-deletes tasks removed from plan" {
    # Create tasks in DB
    "$SCRIPT_DIR/lib/task" create ps-del-01 "Keep this" -r my-spec >/dev/null
    "$SCRIPT_DIR/lib/task" create ps-del-02 "Remove this" -r my-spec >/dev/null
...
@test "plan-sync does not delete done tasks removed from plan" {
    # Create and complete a task
    "$SCRIPT_DIR/lib/task" create ps-keep-01 "Completed work" -p 1 -r my-spec >/dev/null
    "$SCRIPT_DIR/lib/task" create ps-keep-02 "Still open" -p 2 -r my-spec >/dev/null
    export RALPH_AGENT_ID="test-agent"
    "$SCRIPT_DIR/lib/task" claim --agent test-agent >/dev/null
    "$SCRIPT_DIR/lib/task" done ps-keep-01 >/dev/null
    ...
    run "$SCRIPT_DIR/lib/task" show ps-keep-01
    assert_success
...
@test "plan-sync handles multiple spec_refs independently" {
    ...
    "$SCRIPT_DIR/lib/task" create ps-multi-01 "Spec A task 1" -r spec-a >/dev/null
    "$SCRIPT_DIR/lib/task" create ps-multi-02 "Spec A task 2" -r spec-a >/dev/null
    "$SCRIPT_DIR/lib/task" create ps-multi-03 "Spec B task 1" -r spec-b >/dev/null
...
{"id":"ps-multi-03","t":"Spec B task 1","spec":"spec-b"}'
    ...
    run "$SCRIPT_DIR/lib/task" show ps-multi-01
    assert_success
...
@test "plan-sync does not delete tasks without matching spec_ref" {
    ...
    "$SCRIPT_DIR/lib/task" create ps-nospec-01 "No spec task" >/dev/null
    ...
    run "$SCRIPT_DIR/lib/task" show ps-nospec-01
    assert_success
    assert_output --partial "Status:      open"
}
...
@test "plan-sync handles special characters in titles and descriptions" {
    ...
    run "$SCRIPT_DIR/lib/task" show ps-special-01
    assert_success
...
@test "plan-sync performs insert, update, delete, and skip in a single run" {
    ...
    "$SCRIPT_DIR/lib/task" create ps-mix-01 "To update" -p 2 -r mix-spec >/dev/null
    "$SCRIPT_DIR/lib/task" create ps-mix-02 "To delete" -p 2 -r mix-spec >/dev/null
    "$SCRIPT_DIR/lib/task" create ps-mix-03 "Already done" -p 1 -r mix-spec >/dev/null
    export RALPH_AGENT_ID="test-agent"
    "$SCRIPT_DIR/lib/task" claim --agent test-agent >/dev/null
    "$SCRIPT_DIR/lib/task" done ps-mix-03 >/dev/null
...
{"id":"ps-mix-04","t":"Brand new task","p":1,"spec":"mix-spec"}'
    ...
    # Verify results
    run "$SCRIPT_DIR/lib/task" show ps-mix-01
    assert_output --partial "Updated title"
    assert_output --partial "Priority:    0"

    run "$SCRIPT_DIR/lib/task" show ps-mix-02
    assert_output --partial "Status:      deleted"

    run "$SCRIPT_DIR/lib/task" show ps-mix-03
    assert_output --partial "Status:      done"
    assert_output --partial "Already done"

    run "$SCRIPT_DIR/lib/task" show ps-mix-04
...
@test "plan-sync rejects invalid JSON line" {
    run bash -c 'printf "%s\n" "$1" | "$SCRIPT_DIR/lib/task" plan-sync 2>"$TEST_WORK_DIR/stderr"' -- "not json at all"
    [ "$status" -eq 1 ]
...
@test "plan-sync rejects JSON missing id field" {
    run bash -c 'printf "%s\n" "$1" | "$SCRIPT_DIR/lib/task" plan-sync 2>"$TEST_WORK_DIR/stderr"' -- '{"t":"No id"}'
    [ "$status" -eq 1 ]
    grep "line 1" "$TEST_WORK_DIR/stderr"
...
@test "plan-sync rejects JSON with empty id field" {
    run bash -c 'printf "%s\n" "$1" | "$SCRIPT_DIR/lib/task" plan-sync 2>"$TEST_WORK_DIR/stderr"' -- '{"id":"","t":"Empty id"}'
    [ "$status" -eq 1 ]
    grep "line 1" "$TEST_WORK_DIR/stderr"
    grep '"id"' "$TEST_WORK_DIR/stderr"
    grep "missing or empty" "$TEST_WORK_DIR/stderr"
...
@test "plan-sync rejects JSON missing t (title) field" {
    run bash -c 'printf "%s\n" "$1" | "$SCRIPT_DIR/lib/task" plan-sync 2>"$TEST_WORK_DIR/stderr"' -- '{"id":"val-01"}'
    [ "$status" -eq 1 ]
    grep "line 1" "$TEST_WORK_DIR/stderr"
...
@test "plan-sync rejects JSON with empty t (title) field" {
    run bash -c 'printf "%s\n" "$1" | "$SCRIPT_DIR/lib/task" plan-sync 2>"$TEST_WORK_DIR/stderr"' -- '{"id":"val-01","t":""}'
    [ "$status" -eq 1 ]
    grep "line 1" "$TEST_WORK_DIR/stderr"
    grep '"t"' "$TEST_WORK_DIR/stderr"
    grep "missing or empty" "$TEST_WORK_DIR/stderr"
...
@test "plan-sync rejects negative priority" {
    run bash -c 'printf "%s\n" "$1" | "$SCRIPT_DIR/lib/task" plan-sync 2>"$TEST_WORK_DIR/stderr"' -- '{"id":"val-01","t":"Good title","p":-1}'
    [ "$status" -eq 1 ]
    grep "line 1" "$TEST_WORK_DIR/stderr"
...
@test "plan-sync rejects non-integer priority" {
    run bash -c 'printf "%s\n" "$1" | "$SCRIPT_DIR/lib/task" plan-sync 2>"$TEST_WORK_DIR/stderr"' -- '{"id":"val-01","t":"Good title","p":"high"}'
    [ "$status" -eq 1 ]
    grep "line 1" "$TEST_WORK_DIR/stderr"
    grep '"p"' "$TEST_WORK_DIR/stderr"
    grep "non-negative integer" "$TEST_WORK_DIR/stderr"
...
@test "plan-sync rejects float priority" {
    run bash -c 'printf "%s\n" "$1" | "$SCRIPT_DIR/lib/task" plan-sync 2>"$TEST_WORK_DIR/stderr"' -- '{"id":"val-01","t":"Good title","p":1.5}'
    [ "$status" -eq 1 ]
    grep "line 1" "$TEST_WORK_DIR/stderr"
    grep '"p"' "$TEST_WORK_DIR/stderr"
    grep "non-negative integer" "$TEST_WORK_DIR/stderr"
...
@test "plan-sync error identifies correct line number for second line" {
    ...
    local input="${line1}
${line2}"
    run bash -c 'printf "%s\n" "$1" | "$SCRIPT_DIR/lib/task" plan-sync 2>"$TEST_WORK_DIR/stderr"' -- "$input"
    [ "$status" -eq 1 ]
...
${line2}"
    ...
    run bash -c '"$SCRIPT_DIR/lib/task" show val-no-write-01 2>&1'
    [ "$status" -eq 2 ]
...
@test "plan-sync accepts valid input with optional p absent" {
    run bash -c 'printf "%s\n" "$1" | "$SCRIPT_DIR/lib/task" plan-sync' -- '{"id":"val-opt-01","t":"No priority","spec":"my-spec"}'
    assert_success
    assert_output "inserted: 1, updated: 0, deleted: 0, skipped (done): 0"
}
